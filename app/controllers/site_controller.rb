class SiteController < ApplicationController
  ...
  def index
    @interesting_users = InterestingUser.where(:category_type => [1,2].sample).to_a.sample(8).index_by(&:instagram_user_id)
    #redirect_to "/popular" if @interesting_users.blank?
    @interesting_user_posts = InterestingUserPost.where(:instagram_user_id => @interesting_users.keys).load.shuffle.group_by(&:instagram_user_id)
  end
